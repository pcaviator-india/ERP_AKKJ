...
CREATE TABLE IF NOT EXISTS Employees ( -- Can also serve as Users
...
ALTER TABLE Employees
  ADD COLUMN PinHash VARCHAR(255) NULL AFTER PasswordHash;
...
CREATE TABLE IF NOT EXISTS POSUserSettings (
...
CREATE TABLE IF NOT EXISTS DigitalCertificates ( -- For DTE signing
...
CREATE TABLE IF NOT EXISTS CompanySubscribedAddons (
...
CREATE TABLE IF NOT EXISTS FeatureLimits (
...
CREATE TABLE IF NOT EXISTS PlanFeatureLimits (
...
CREATE TABLE IF NOT EXISTS CompanyCustomFeatureLimits (
...
CREATE TABLE IF NOT EXISTS CompanyUsageRecords (
...
CREATE TABLE ProductImages (
    ...
    CONSTRAINT UQ_ProductImages_Primary UNIQUE (ProductID, IsPrimary)
        /*!80000 INVISIBLE */  -- optional: remove if your MySQL doesn’t support invisible indexes
);
...
CREATE TABLE IF NOT EXISTS DocumentSequences ( -- Folios
...
-- CAF storage
CREATE TABLE IF NOT EXISTS SiiCAF (
...
-- DTE logs and queues
CREATE TABLE IF NOT EXISTS SiiDteLogs (
...
CREATE TABLE IF NOT EXISTS SiiSendQueue (
...
CREATE TABLE IF NOT EXISTS SiiRejectedDocs (
...
CREATE TABLE IF NOT EXISTS Tickets (
    ...
    BilledByEmployeeID INT NULL,
    TicketNumber INT NOT NULL,
    Status ENUM('OPEN','IN_PROGRESS','BILLED','CANCELLED','EXPIRED') DEFAULT 'OPEN',
    ...
    LinkedSaleID INT NULL,              -- final BOLETA/FACTURA/GUIA once billed
    Notes VARCHAR(500),

    CreatedAt DATETIME DEFAULT CURRENT_TIMESTAMP,
    UpdatedAt DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    CONSTRAINT FK_Tickets_Companies
        FOREIGN KEY (CompanyID) REFERENCES Companies(CompanyID) ON DELETE CASCADE,
    CONSTRAINT FK_Tickets_Customers
        FOREIGN KEY (CustomerID) REFERENCES Customers(CustomerID),
    CONSTRAINT FK_Tickets_Employees_Created
        FOREIGN KEY (CreatedByEmployeeID) REFERENCES Employees(EmployeeID),
    CONSTRAINT FK_Tickets_Employees_Billed
        FOREIGN KEY (BilledByEmployeeID) REFERENCES Employees(EmployeeID),
    CONSTRAINT FK_Tickets_Sales
        FOREIGN KEY (LinkedSaleID) REFERENCES Sales(SaleID),
    CONSTRAINT FK_Tickets_Warehouses
        FOREIGN KEY (WarehouseID) REFERENCES Warehouses(WarehouseID),
    CONSTRAINT UQ_Tickets_Company_TicketNumber
        UNIQUE (CompanyID, TicketNumber)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;



-- Ticket items for ticket creation

CREATE TABLE IF NOT EXISTS TicketItems (
    TicketItemID INT AUTO_INCREMENT PRIMARY KEY,
    ...
    CONSTRAINT FK_TicketItems_Tickets
        FOREIGN KEY (TicketID) REFERENCES Tickets(TicketID) ON DELETE CASCADE,
    CONSTRAINT FK_TicketItems_Products
        FOREIGN KEY (ProductID) REFERENCES Products(ProductID),
    CONSTRAINT FK_TicketItems_Lot
        FOREIGN KEY (ProductLotID) REFERENCES ProductLots(ProductLotID),
    CONSTRAINT FK_TicketItems_Serial
        FOREIGN KEY (ProductSerialID) REFERENCES ProductSerials(ProductSerialID),
    CONSTRAINT CK_TicketItems_Tax_Exenta
        CHECK ((IsLineExenta = 1 AND TaxAmountItem = 0) OR (IsLineExenta = 0))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS JournalEntryItems (
...
-- Books for SII
CREATE TABLE IF NOT EXISTS LibroVentas (
...
CREATE TABLE IF NOT EXISTS LibroCompras (
...
CREATE TABLE IF NOT EXISTS PayslipItems (
...
CREATE TABLE IF NOT EXISTS CompanyMarketplaceConnections (
...
CREATE TABLE IF NOT EXISTS ProductMarketplaceMappings (
...
CREATE TABLE IF NOT EXISTS OrderMarketplaceMappings (
...
CREATE TABLE IF NOT EXISTS MarketplaceSyncLogs (
...
CREATE TABLE IF NOT EXISTS CompanyPaymentGatewayConnections (
...
-- Tax rates table (company-scoped)
...
-- Link products to a tax rate (nullable)
ALTER TABLE Products
  ADD COLUMN IF NOT EXISTS TaxRateID INT NULL DEFAULT NULL;

-- Store tax rate used on each sales line for history
...
CREATE TABLE IF NOT EXISTS promotions (
  ...
  MinQuantity       INT NULL,          -- trigger by quantity
...
ALTER TABLE promotions ADD COLUMN CompanyID BIGINT NOT NULL DEFAULT 0;
ALTER TABLE promotions ADD INDEX IX_Promotions_Company (CompanyID);
ALTER TABLE promotions ADD COLUMN UnitPrice DECIMAL(18,4) NULL AFTER Value;


-- Scopes (link promotion to products/categories/brands/customers/employees/custom fields/channels/days)
...
-- Optionally track usages/redemptions
